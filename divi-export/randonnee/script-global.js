...
window.closePopup = function(){
  ...
  setTimeout(function(){
    showStep(1);
    var f=document.getElementById('inscriptionForm');
    if(f) f.reset();
...
window.goStep2 = function(e){
  ...
  data.append('action','ust_inscription_randonnee');
...
function initStripePayment(){
  ...
  fetch(UST_CONFIG.paymentIntentUrl,{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({amount:8000,currency:'eur',description:'Cotisation Randonnée UST 2025/2026',email:document.getElementById('insc-email').value})
  })
  .then(function(r){return r.json();})
  .then(function(data){
  ...
  .catch(function(){
    loading.innerHTML='<p style="color:#dc2626;font-size:13px">⚠️ Erreur chargement paiement. <a href="https://boutique.ustoulouse.com/produit/cotisation-randonnee" target="_blank" style="color:var(--bleu)">Payer via la boutique →</a></p>';
  });
}
...
window.confirmPayment = function(){
  ...
  stripeInstance.confirmPayment({
    ...
    if(result.error){
      ...
      btn.textContent='Payer 80€ en sécurisé';btn.disabled=false;
    } else {showStep(3);}
  });
};
